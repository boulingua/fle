...
**Which gate proves fle's conformance claim?**
The reusable workflow `boulingua/.github/.github/workflows/course-build.yml@v1` runs `python .curriculum/scripts/conformance_audit.py resolve --manifest conformance.yml --content content`. The framework's own level-file audit script validates statements **inside the curriculum repo's own `levels/*.md`**; it is not a consumer gate, it takes no manifest argument, and pointed at fle it would audit the framework and exit 0 whatever fle contains. **Do not wire it here.** The consumer obligation (from `examples/de-a1/README.md` + `docs/verification.md`) is that **every `implements_id` in fle's `conformance.yml` must resolve to a real statement in `curriculum/levels/`**. FLE has no such file, so there is nothing to resolve today. The task is to (a) produce the mapping, and (b) run the consumer gate above against a pinned checkout of `curriculum/levels/`.
